...
fn build_query(n: usize) -> ConjunctionQuery {
    ...
    ConjunctionQuery::new(vec![Box::new(q1), Box::new(q1b)])
}
...
fn percolate_real(c: &mut Criterion) {
    ...
    for nqueries in [100, 10000, 50000] {
        ...
        fn build_document(nqueries: usize) -> Document {
        ...
        group.bench_function(BenchmarkId::new("perc_dyna", &p), |b| {
            b.iter_batched(
                || build_document(nqueries),
                |d| black_box(p.qids_from_document(&d).next().unwrap()),
                input_size,
            )
        });
        ...
        group.bench_function(BenchmarkId::new("multi_perc", &mp), |b| {
            b.iter_batched(
                || build_document(nqueries),
                |d| black_box(mp.bs_qids_from_document(&d).next().unwrap()),
                input_size,
            )
        });
    }
